...
void ProblemMaster::solveGomory(CEnv env, Prob lp) {
	...
	// --------------------------------------------------
	if (stat == CPX_STAT_UNBOUNDED || stat == CPX_STAT_INFEASIBLE) {
		cout << endl << " STOP PROBLEM MASTER IS UNBOUNDED OR INFEASIBLE "
				<< endl;
		cout << " Iteration number: " << iter << endl;
		throw std::runtime_error("  STOP CONDITION STEP 1 ");
	}
...
void ProblemMaster::add_constraint_Gomory(CEnv env, Prob lp, double* gamma,
		int var, int aggressivity) {
